...
class CRM_Orgrelationshiptab_Page_Ajax {
  ...
  // but get only relationship with an organization
  public function getOrgRelationships() {
    ...
    Civi::log()->debug('getOrgRelationships -- ' . print_r($relationships,1));

    // only filter for organization -- should be called for organization anyway
    if (CRM_Contact_BAO_Contact::getContactType($contactID) == 'Organization') {
      ...
      foreach ($relationships['data'] as $idx => $relationship) {
        if (in_array($relationship['DT_RowId'], $excludeIds)) {
          // exclude that relationship as it should be in the other tab
